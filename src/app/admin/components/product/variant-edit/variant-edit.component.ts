...
export class VariantEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
     ...
     this.service.GetOneVariant(this.id,this.cookie.get('token')).subscribe((data)=>{this.variant=data;
      this.form.setValue({
  ...
  onSubmit(){
    this.service.updateVariant(this.form.value,this.cookie.get('token')).subscribe((data)=>{
      this.location.back();
    })
